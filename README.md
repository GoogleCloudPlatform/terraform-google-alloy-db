...
| cluster\_timeouts | (Optional) Timeout configuration for the AlloyDB cluster | <pre>object({<br>    create = optional(string)<br>    update = optional(string)<br>    delete = optional(string)<br>  })</pre> | `null` | no |
...
| dataplex\_config | Configuration for Dataplex integration. The `enabled` flag controls the integration of AlloyDB PG resources (like databases, schemas, and tables) with Dataplex. See https://docs.cloud.google.com/alloydb/docs/dataplex-catalog-integration for more details. | <pre>object({<br>    enabled = bool<br>  })</pre> | `null` | no |
...
- [Terraform Provider for GCP][terraform-provider-gcp] plugin >= v7.23+
