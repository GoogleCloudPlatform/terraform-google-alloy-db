...
func TestPscExample(t *testing.T) {
	example := tft.NewTFBlueprintTest(t)

	example.DefineVerify(func(assert *assert.Assertions) {
		// Getting the expected Project ID from the outputs
		projectId := example.GetStringOutput("project_id")
		kmsKeyName := example.GetStringOutput("kms_key_name_central")
		secondarykmsKeyName := example.GetStringOutput("kms_key_name_east")
		region := example.GetStringOutput("region_central")
		secondaryRegion := example.GetStringOutput("region_east")

		// Cluster Information
		alloydbClusterIdPath := example.GetStringOutput("cluster_id_central")
		...
		secondaryAlloydbClusterIdPath := example.GetStringOutput("cluster_id_east")
